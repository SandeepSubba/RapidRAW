...
export default function MasksPanel() {
  ...
  const handleAddComponentModeMenu = (event: React.MouseEvent, containerId: string, mode: SubMaskMode) => {
    ...
    const primaryTypes = [
      ...MASK_AI_TYPES.filter((m) => m.type !== Mask.AiDepth),
      ...MASK_BASIC_TYPES.filter((m) => m.type === Mask.Linear || m.type === Mask.Radial),
    ];
    const options: any[] = primaryTypes.map(toOption);
    options.push({
      label: getMaskTypeName({ disabled: false, icon: MoreHorizontal, id: 'others', name: 'Others', type: null as any }),
      icon: MoreHorizontal,
